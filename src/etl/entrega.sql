...
SELECT idVendedor,
    ...
    avg(
        julianday(coalesce(dtEntregue, '2018-01-01')) - julianday(dtAprovado)
    ) AS qtdDiasAprovadoEntrega,
    avg(
        julianday(coalesce(dtEntregue, '2018-01-01')) - julianday(dtPedido)
    ) AS qtdDiasPedidoEntrega,
    avg(
        julianday(dtEstimativaEntrega) - julianday(coalesce(dtEntregue, '2018-01-01'))
